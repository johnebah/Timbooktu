...
@section('content')
  @php
    $featuredImageUrl = isset($featuredPost) && $featuredPost?->image_path
      ? \Illuminate\Support\Facades\Storage::url($featuredPost->image_path)
      : asset('img/abt.jpeg');
    $featuredAudioUrl = isset($featuredPost) && $featuredPost?->audio_path
      ? \Illuminate\Support\Facades\Storage::url($featuredPost->audio_path)
      : null;
          ...
          @if (isset($photographs) && $photographs->count())
            @foreach ($photographs as $photo)
              <div class="fotografie-item">
                <img src="{{ \Illuminate\Support\Facades\Storage::url($photo->image_path) }}" alt="{{ $photo->title ?: 'Photo' }}" />
              </div>
            @endforeach
          @else
            <div class="fotografie-item">
